function phi = GEVLevelSet(phi_0, vol, g, mu, lambda, alpha, beta_v, gamma_v, epsilon, timestep, iter)
% Input:
...
% This script models the bone region as the Generalized Extreme Value distribution and use MLE
% to estimate the shape, location and scale. Brent iterative updating is used for 
% solving the equation. Shape parameter 'sh', scale parameter 'sc' and location
% parameter 'lc' 
...
for i = 1 : iter
    ...
    param = GEVParam(vol, fore_mask);
    sh = param(1); sc = param(2); lc = param(3);
    fprintf('shape & scale & location: %f, %f, %f. \n', sh, sc, lc);
    
    M = 100;
    phi_mask = single((phi >= -M) & (phi <= M));
    
    inner_log_vol = LogVol(1 + sh * (vol - lc) / sc);
    inter_term = log(sc) + (1 + sh * (vol - lc) / sc).^(-1 / sh) + (1 + 1 / sh) * inner_log_vol;
...
function para = GEVParam(vol, mask)
[vx, vy, vz] = size(vol);
