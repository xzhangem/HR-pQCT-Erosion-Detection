function phi = WeibullLevelSet(phi_0, vol, g, mu, lambda, alpha, beta_v, gamma_v, epsilon, timestep, iter)
% Input:
...
% This script models the bone region as the Weibull distribution and use MLE
...
for i = 1 : iter
    ...
    inner_shift = min(min(min(vol .* heavi_phi)));
    fore_mask = single(heavi_phi > 0);
    param = WblParam(vol, fore_mask, inner_shift);
    beta = param(2); eta = param(1);
    fprintf('beta & eta: %f, %f. \n', beta, eta);
    inner_log_vol = LogVol((vol - inner_shift) / eta);
    
    M = 100;
    phi_mask = single((phi >= -M) & (phi <= M));
   
    inter_term = -log(beta / eta) - (beta - 1) * inner_log_vol + ((vol - inner_shift) / eta).^(beta);
...
function para = WblParam(vol, mask, min)
[vx, vy, vz] = size(vol);
