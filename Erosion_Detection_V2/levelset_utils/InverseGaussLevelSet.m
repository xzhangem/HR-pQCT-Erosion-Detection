function phi = InverseGaussLevelSet(phi_0, vol, g, mu, lambda, alpha, beta, gamma, epsilon, timestep, iter)
% Input: 
...
%   beta: weight of the weighted internal Inverse Gaussian
...
for k = 1 : iter
    ...
    % Calculate mu and lambda in the internal area of Inverse Gaussian. 
    % mu: arithmetic mean; lambda: reciprocal of the difference between
    % arithmetic mean and harmonic mean of the reciprocal of the volume
    % intensity. 
    ...
    inter_term = -log(in_lambda + small_number) + 3 * log(vol + small_number) + (in_lambda * (vol - in_mu).^2) ...
        ./ (in_mu^2 * (vol + small_number));
