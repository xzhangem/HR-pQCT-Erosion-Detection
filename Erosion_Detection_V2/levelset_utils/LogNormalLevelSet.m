function phi = LogNormalLevelSet(phi_0, vol, g, mu, lambda, alpha, beta, gamma, epsilon, timestep, iter)
% Input: 
...
%   beta: weight of the weighted internal Log-Normal
...
for k = 1 : iter
    ...
    % Calculate mu and lambda in the internal area of Log-Normal. 

    in_mu = (int3D(log(vol) .* heavi_phi)) / (int3D(heavi_phi));
    in_lambda_square = (int3D((log(vol) - in_mu).^2)) / (int3D(heavi_phi));
    in_lambda = sqrt(in_lambda_square);
    inter_term = ((log(vol) - in_mu).^2 / 2 * (in_lambda_square + small_number) + log(in_lambda * vol));
